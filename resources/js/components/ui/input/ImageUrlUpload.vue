...
const removeFile = () => {
  ...
  // Parent component'a silme eventini gönder
  emit('remove')
}
...
// Yeni: Dosya adından alt text başlangıç değeri üret
watch(selectedFile, (file) => {
  if (
    file &&
    (!props.altText || props.altText === '') // Eğer altText boşsa
  ) {
    // Dosya adını uzantısız al ve - gibi karakterleri kaldır
    let nameWithoutExt = file.name.replace(/\.[^/.]+$/, '');
    nameWithoutExt = nameWithoutExt.replace(/[-_]+/g, ' ').trim(); // - ve _ karakterlerini boşluk yap
    emit('update:altText', nameWithoutExt);
  }
})
